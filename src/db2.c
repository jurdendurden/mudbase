...
void load_quests (FILE * fp)
{
	...
	for (;;)
    {
		...
		pQuest->min_level = fread_number 			(fp);
		pQuest->max_level = fread_number 			(fp);
		pQuest->accept_vnum = fread_number 		(fp);
		pQuest->return_vnum = fread_number	 	(fp);
		pQuest->ch_class_type = fread_number 		(fp);
		pQuest->type = fread_flag				(fp);
		pQuest->gold_reward = fread_number		(fp);
		pQuest->gp_reward = fread_number		(fp);
		pQuest->xp_reward = fread_number		(fp);		
		pQuest->target_obj_vnum = fread_number (fp);
		pQuest->target_mob_vnum = fread_number (fp);
		pQuest->target_mat_index = fread_number (fp);
		pQuest->target_herb_index = fread_number (fp);
		pQuest->obj_mult = fread_number (fp);
		pQuest->mob_mult = fread_number (fp);
		pQuest->mat_mult = fread_number (fp);
		pQuest->herb_mult = fread_number (fp);
		pQuest->help1 = fread_string (fp);
		pQuest->help2 = fread_string (fp);
		pQuest->help3 = fread_string (fp);
		
		int i = 0;
		for (i = 0; i < 4; i++)
		{
			pQuest->quest_requisite[i] = fread_number(fp);
		}
		
		pQuest->desc = fread_string				(fp);
	...
	log_f("Loaded %d quests.", top_quest);
    return;
...
void load_mobiles (FILE * fp)
{
    ...
    for (;;)
    {
		...
		if (!area_last->deprecated)
			pMobIndex->walk_desc = fread_string (fp);
		else 
			pMobIndex->walk_desc = "";

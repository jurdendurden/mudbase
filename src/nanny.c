    ...
    switch (d->connected)
    {
		...
		case CON_CONFIRM_NEW_ACC_NAME:
			...
			case 'Y':
				...
				//Verify all accounts for now while in Alpha. - Upro 1/31/2020
				d->account->act = d->account->act | ACC_NEWBIE; 
				d->account->act = d->account->act | VERIFY; 
				
				//Put a secret flag here for a rare race.
...
/*
		case CON_GET_EMAIL: // if you change this, need to change it in account.c send_mail as well.
			...
			else 
			{
				...
				{
					write_to_buffer( d, "Remember to contact Upro to\n\r", 0);
					write_to_buffer( d, "get your account verified.  If you don't,\n\r", 0);
					write_to_buffer( d, "you won't be limited in play.\n\r",0);	
				}
			}
...
/*
		case CON_VERIFY_EMAIL:
			if ((argument[0] == 'y') || (argument[0] == 'Y'))
			{
					...
					write_to_buffer( d, "Remember to choose something hard to guess,\n\r", 0);
					write_to_buffer( d, "because this allows access to all your characters.\n\r", 0);
					write_to_buffer( d, "Password:  ", 0);
					write_to_buffer( d, echo_off_str, 0);
			d->connected = CON_GET_PASS;
			return;	
			...
			write_to_buffer( d, "NULL as your email address, then write a note to\n\r", 0);
					write_to_buffer( d, "powerimms about it, or talk to Upro\n\r", 0);
			write_to_buffer( d, "to get your account approved.\n\r", 0);
			write_to_buffer( d, "\n\rEmail address:  ", 0);
...
*/
		...
		case CON_ACCOUNT:
			...
			STD("{r=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-={x\n\r",d);
			STD("{r={x You are creating a new character to live, explore, and interact within      {r={x\n\r",d);
			STD("{r={x a roleplaying environment with other real live players.  Please put some    {r={x\n\r",d);
			STD("{r={x amount of thought into your name.  If you are unfamiliar with our naming    {r={x\n\r",d);
			STD("{r={x rules, please consult 'help name'   In general:  video game, current        {r={x\n\r",d);
			STD("{r={x event, historical figure, popular characters from books, culture,           {r={x\n\r",d);
			STD("{r={x compound words, actual words, or words with a letter changed etc            {r={x\n\r",d);
			STD("{r={x are illegal.                                                                {r=\n\r",d);
			STD("{r=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-={x\n\r",d);
			write_to_buffer(d,"\n\r\n\r",0);
		...
		case CON_PLAY_CHAR:
			...
			{ 
						...
						wiznet(log_buf,NULL,NULL,WIZ_SITES,0,UMAX(50,get_trust(ch)));
			}
					else 
					{
					sprintf( log_buf, "%s@%s has connected", ch->name, d->host);
                ...
                case 'Y':				
					...
					if (d->account)
					{
						...
						for(i=0;i<MAX_ACCOUNT_CHAR; i++)
						{
							if(IS_NULLSTR(d->account->char_list[i].char_name))
							{
								d->account->char_count = i+1;																	
								d->account->char_list[i].char_name = str_dup(ch->name);
								// Save the name in an account slot so we save the data when we save
								free_string(d->account->char_list[i].char_name);
								d->account->char_list[i].char_name = str_dup(ch->name);																
								break;
							}
						}
...
//Alert Immortals of new players:					
			...
			case CON_GET_NEW_RACE:											
				...
				else
				{
					if (race && race_table[race].pc_race)
					{
						...
						STD("Would you like to customize this character's appearance?\r\n",d);
						STD ("You can also type {Bback{x at any time to return to the previous menu.\r\n", d);												
						d->connected = CON_CUSTOMIZE_APPEARANCE;
						break;
					}
					else
						return;
		...
		case CON_CUSTOMIZE_APPEARANCE:
			switch(argument[0])
			{
				...
				case 'N':
				case 'n':
					STD ("Alright, please choose a class for your character:\r\n",d);
		...
		case CON_GET_NEW_CLASS:
			...
			else
			{
				if (IS_PALADIN(ch))
				{
					if (IS_DUERGAR(ch) || IS_DROW(ch) || IS_MINOTAUR(ch) || IS_BUGBEAR(ch))
					{
		...
		case CON_GET_ALIGNMENT:
			switch (argument[0])
			{
				...
				case 'g':
				case 'G':
					if (/*pc_race_table[ch->race].can_good == FALSE ||*/ IS_DROW(ch) || IS_REVENANT(ch) || IS_BUGBEAR(ch) || IS_HALF_OGRE(ch) || IS_BIRDFOLK(ch))
					{
				...
				case 'n':
				case 'N':
					if (/*pc_race_table[ch->race].can_neutral == FALSE ||*/ IS_AASIMAR(ch) || IS_REVENANT(ch))
					{
				...
				case 'e':
				case 'E':
					if (/*pc_race_table[ch->race].can_evil == FALSE ||*/ IS_AASIMAR(ch))
					{
		...
		case CON_GET_ETHOS:
			...
			switch (argument[0])
			{
				...
				case 'C':
					if (IS_LOXODON(ch))
					{
						STD("Loxodon are inherently lawful in their ethos.\r\n",d);
						STD("Which ethos {r({xL{r){x?\r\n", d);
						return;
					}
					if (ch->alignment == 500)
					{
		...
		case CON_GET_BOON:				
			...
			{
				case '\0':
					STD("Please pick a boon or {Bskip{x to continue.\r\n",d);
					d->connected = CON_GET_BOON;
					return;
				case 'b':
				case 'B':
			...
			{
				if ((god_table[god].pc_evil == TRUE && IS_EVIL(ch))
				|| (god_table[god].pc_neutral == TRUE && IS_NEUTRAL(ch))
				|| (god_table[god].pc_good == TRUE && IS_GOOD(ch)))
				{
					if((god_table[god].pc_wizard == TRUE && ch->ch_class == 0)
					|| (god_table[god].pc_cleric == TRUE  && ch->ch_class == 1)
					|| (god_table[god].pc_thief == TRUE  && ch->ch_class == 2)
					|| (god_table[god].pc_fighter == TRUE  && ch->ch_class == 3)
					|| (god_table[god].pc_ranger == TRUE  && ch->ch_class == 4)
					|| (god_table[god].pc_druid == TRUE  && ch->ch_class == 5)
					|| (god_table[god].pc_paladin == TRUE  && ch->ch_class == 6)
					|| (god_table[god].pc_bard == TRUE  && ch->ch_class == 7)
					|| (god_table[god].pc_monk == TRUE  && ch->ch_class == 8))
					{
        ...
        case CON_GET_GOD:
			...
			if ( god == -1
				|| (god_table[god].pc_evil == FALSE && IS_EVIL(ch))
				|| (god_table[god].pc_neutral == FALSE && IS_NEUTRAL(ch))
				|| (god_table[god].pc_good == FALSE && IS_GOOD(ch))
				|| (god_table[god].pc_wizard == FALSE && ch->ch_class == 0)
				|| (god_table[god].pc_cleric == FALSE && ch->ch_class == 1)
				|| (god_table[god].pc_thief == FALSE && ch->ch_class == 2)
				|| (god_table[god].pc_fighter == FALSE && ch->ch_class == 3)
				|| (god_table[god].pc_ranger == FALSE && ch->ch_class == 4)
				|| (god_table[god].pc_druid == FALSE && ch->ch_class == 5)
				|| (god_table[god].pc_paladin == FALSE && ch->ch_class == 6)
				|| (god_table[god].pc_bard == FALSE && ch->ch_class == 7)
				|| (god_table[god].pc_monk == FALSE && ch->ch_class == 8)
				)		
			{

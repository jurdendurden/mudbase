...
#define NOTE_DIR  		"../notes"  		// Note directory
...
{ "General",	"General Discussion",				0,	2,		"all", DEF_INCLUDE,	2000, 	NULL, FALSE },
{ "Changes",	"Game Changes and Updates",			0,	L_IMM,	"all", DEF_NORMAL,  2000,   NULL, FALSE },
{ "Ideas",		"Suggestions for Improvement",		0,	2,		"all", DEF_NORMAL, 	2000, 	NULL, FALSE }, 
{ "Announce",	"Announcements from Immortals",		0,	L_IMM,	"all", DEF_NORMAL, 	2000, 	NULL, FALSE },
{ "Bugs",		"Typos, Bugs, and Errors",			0,	1,		"imm", DEF_NORMAL,	2000, 	NULL, FALSE },
{ "Roleplay",	"Roleplays, Stories, Etc.",			0,	1,		"all", DEF_NORMAL,	2000, 	NULL, FALSE },
{ "Personal",	"Personal Messages",				0,	1,		"all", DEF_EXCLUDE,	2000, 	NULL, FALSE },
...
static void do_nwrite (CHAR_DATA *ch, char *argument)
{
	...
	printf_to_char (ch, "You are now %s a new note on the {W%s{x board.\r\n"
	              "If you are using tintin, type #verbose to turn off alias expansion!\r\r\n\n",
	               ch->pcdata->in_progress->text ? "continuing" : "posting",
	               ch->pcdata->board->short_name);
	
	printf_to_char (ch, "{YFrom{x:    %s\r\r\n\n", ch->name);
	...
	else /* we are continuing, print out all the fields and the note so far*/
	{
		...
		SEND ("{GYour note so far:{x\r\n", ch);
		SEND (ch->pcdata->in_progress->text,ch);
...
static void do_nremove (CHAR_DATA *ch, char *argument)
{
	...
	{
		SEND ("You are not authorized to remove this note.\r\n",ch);
		return;
	}
...
void do_note (CHAR_DATA *ch, char *argument)
{
	...
	else if (!str_cmp (arg, "write"))
	{
		...
		{
			SEND("You can't write notes while in OLC.\r\n",ch);
			return;
		}
		if (ch->fighting != NULL)
		{
			SEND("You can't write notes while fighting!\r\n",ch);
			return;
		}
...
void do_board (CHAR_DATA *ch, char *argument)
{
	...
	if (!argument[0]) /* show boards */
	{
		...
		count = 1;
			SEND ("{xNum          Name Unread Description{x\r\n"
		              "{r---- ------------ ------ -----------------------------{x\r\n",ch);
		for (i = 0; i < MAX_BOARD; i++)
		{
			...
			{ 
				printf_to_char (ch, "({W%2d{x) {y%12s{x [%s%4d{x] %s{x\r\n", 
		...
		printf_to_char (ch, "\r\nYour current board is {W%s{x.\r\n", ch->pcdata->board->short_name);

		/* Inform of rights */		
		if (ch->pcdata->board->read_level > get_trust(ch))
			SEND ("You cannot read nor write notes on this board.\r\n",ch);
		else if (ch->pcdata->board->write_level > get_trust(ch))
			SEND ("You can only read notes from this board.\r\n",ch);
		else
			SEND ("You can both read and write on this board.\r\n",ch);
    ...
    if (ch->pcdata->in_progress)
    {
    	SEND ("Please finish your interrupted note first.\r\n",ch);
        return;
    }
...
void handle_con_note_subject (DESCRIPTOR_DATA *d, char * argument)
{
	...
	{
		write_to_buffer (d, "Try to find a shorter subject.\r\n",0);
	}
	else
	/* advance to next stage */
	{
		...
		if (IS_IMMORTAL(ch)) /* immortals get to choose number of expire days */
		{
			...
			//d->character->note_state = NOTE_EXPIRE;
			//note_nanny(d, argument);
		...
		else
		{
			...
			send_to_desc ("\r\nEnter text. Type {W~{x or {WEND{x on an empty line to end note.\r\n"
			                    "=======================================================\r\n",d);
			//d->character->note_state = NOTE_TEXT;
			//note_nanny(d, argument);
...
void handle_con_note_text (DESCRIPTOR_DATA *d, char * argument)
{
	...
	if ((!str_cmp(buf, "~")) || (!str_cmp(buf, "END")))
	{
		...
		write_to_buffer (d, "\r\n", 0);
		//d->character->note_state = NOTE_FINISH;
		//note_nanny(d, argument);
		...
		switch (tolower(argument[0]))
		{
			...
			case 'v': /* view note so far */
				if (ch->pcdata->in_progress->text)
				{
					...
					write_to_buffer (d,"You haven't written a thing!\r\r\n\n",0);
				//printf_to_desc (d, "%s", szFinishPrompt);
				write_to_buffer (d, "\r\n",0);
				break;
			...
			default: /* invalid response */
				write_to_buffer (d, "Huh? Valid answers are:\r\r\n\n",0);
				//printf_to_desc (d, "%s", szFinishPrompt);
				write_to_buffer (d, "\r\n",0);

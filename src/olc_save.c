...
void save_quest ( FILE * fp, QUEST_INDEX_DATA * pQuest )
{
	fprintf (fp, "#%ld\n", 	pQuest->vnum);
	fprintf (fp, "%s~\n", 	pQuest->name);
	fprintf (fp, "%d\n",  	pQuest->min_level);
...
void save_mobile (FILE * fp, MOB_INDEX_DATA * pMobIndex)
{
	...
	if (!pMobIndex->form)
		pMobIndex->form |= race_table[RACE_HUMAN].form;
	if (!pMobIndex->parts)
		pMobIndex->parts |= race_table[RACE_HUMAN].parts;
    ...
    switch (pObjIndex->item_type)
    {
		...
		case ITEM_INSTRUMENT:
            fprintf( fp, "%s %d %d %d %d\n",
		     instrument_name( pObjIndex->value[0] ),
                     pObjIndex->value[1],
                     pObjIndex->value[2],
                     pObjIndex->value[3],
                     pObjIndex->value[4]);
...
void save_area (AREA_DATA * pArea)
{
	...
	//This will allow me to pull in old school area files and have them automagically update to my format, 
	//no matter how old they are, no matter how much I upgrade this mud. Upro 1/7/2020
	pArea->deprecated = FALSE;
	fprintf (fp, "Deprecated %d\n", pArea->deprecated);
...
/*    {
    ...
    if (arg1[0] == '\0')
    {
        ...
        {
            SEND ("Syntax:\r\n", ch);
            SEND ("  asave <vnum>   - saves a particular area\r\n", ch);
            SEND ("  asave list     - saves the area.lst file\r\n", ch);
            SEND
                 ("  asave area     - saves the area being edited\r\n", ch);
            SEND ("  asave changed  - saves all changed zones\r\n", ch);
            SEND ("  asave world    - saves the world! (db dump)\r\n", ch);
			SEND( "  asave help     - saves the help files\r\n", ch );
            SEND ("\r\n", ch);
        }
    ...
    if (is_number (arg1))
    {
        ...
        {
            SEND ("You are not a builder for this area.\r\n", ch);
            return;
        }
    ...
    if (!str_cmp (arg1, "area"))
    {
        ...
        {
            SEND ("You are not editing an area, "
	...
	/* Save Help File */
    if(!str_cmp(arg1, "help") || !str_cmp(arg1, "helps"))
    {
        save_helps();
        SEND( "Help Files Saved.\r\n", ch);
        return;
...
void save_helps()
{
    ...
    if (! (fp = fopen( "help.are", "w") ) )
    {
        ...
        for ( pHelp = help_first; pHelp != NULL; pHelp = pHelp->next )
        {
            ...
            fprintf(fp, "%d %s~\n\n%s~\n%s~\n",
                    pHelp->level, pHelp->keyword, fix_string(pHelp->text), !IS_NULLSTR(pHelp->last_editted_by) ? pHelp->last_editted_by : "");
        }

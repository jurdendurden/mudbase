...
// char * telnet_negotiate (char * command, int protocol)
// {
	// char * negotiation = NULL;	
	
	// bzero(negotiation, sizeof(negotiation));
	
	
	
	
	// negotiation = str_dup(IAC);
...
void do_mxp(CHAR_DATA *ch, char *argument)
{
  ...
  if (!ch->desc->mxp)
  {
    SEND("Sorry, your client either does not support MXP or you\n\r", ch);
	SEND("must relog to enable it.\n\r", ch);
    return;
  }
  if (IS_SET(ch->act, PLR_MXP))
  {
    REMOVE_BIT(ch->act, PLR_MXP);
	shutdown_mxp(ch->desc);
...
static void SendMSSP( descriptor_t *apDescriptor )
{
   ...
   static MSSP_t MSSPTable[] =
   {
      ...
      { "PORT",               "9000" },
      { "CODEBASE",           "Extremely Heavily Modified QuickMUD/ROM 2.4" },
